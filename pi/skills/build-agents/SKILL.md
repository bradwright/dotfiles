---
name: build-agents
description: Multi-agent build supervisor. Runs Planner → Parallel Implementers → Auto-Reviewer → Merge Agent using git worktrees and artifact-driven coordination.
---
...
The output of the `/plan` extension (which runs the `plan-mode` skill) is the required input. The planner must consume the approved `plan.md` from the `/plan` run, then break it into implementer-sized units.
